...
NodoMemoria* dividirmemoria(NodoMemoria* nodo, int tamanio_proceso) {
    ...
    if (nodo->tamanio == tamanio_proceso || (nodo->tamanio > tamanio_proceso && nodo->tamanio - tamanio_proceso <= 20)) {
        nodo->libre = false;
        return nodo;
    }
    ...
    if (nodo->tamanio > tamanio_proceso) {
        if (nodo->left == nullptr && nodo->right == nullptr) {
            nodo->left = CreaArbol(nodo->tamanio / 2);
            nodo->right = CreaArbol(nodo->tamanio / 2);
        }

        NodoMemoria* resultado_izquierdo = dividirmemoria(nodo->left, tamanio_proceso);
        if (resultado_izquierdo != nullptr) {
            return resultado_izquierdo;
        }

        NodoMemoria* resultado_derecho = dividirmemoria(nodo->right, tamanio_proceso);
        if (resultado_derecho != nullptr) {
